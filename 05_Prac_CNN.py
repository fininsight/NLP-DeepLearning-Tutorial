...
class CNN() :
    ...
    def convolution(self, images, n_F, F, S, P=0) :
    ...
    def maxpooling(self, map, pooling) :
    ...
    def fullyconected(self, ) :
        pass


    def train(self, data, layes) :
...
if __name__ == "__main__" :
    ...
    cnn = CNN()
    cnn.train([train_images[:2]], 
        [
            {'layer':'CONV','n_F':32, 'F':(3,3), 'S':(1,1)},
            {'layer':'POOL','pooling':(2,2)},
            {'layer':'CONV','n_F':64, 'F':(3,3), 'S':(1,1)},
            {'layer':'POOL','pooling':(2,2)}
            {'layer':'FLAT'}
            {'layer':'FC','dense':64}
            {'layer':'FC','dense':10}
